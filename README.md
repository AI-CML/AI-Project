...
>
> 학습 데이터와 직접 수집한 실전 테스트 데이터 간의 YAML 클래스 ID 구조 불일치(Label Mismatch)가 발생했습니다. 
> 강제 레이블 변환 시 발생할 수 있는 평가 지표의 왜곡을 방지하기 위해, 정량적 수치(mAP) 평가는 **Val 데이터셋**을 기준으로 통일했습니다. 
>
> 대신, Test 데이터는 수치화에 얽매이지 않고 실제 주행 영상에서 바운딩 박스가 정확히 매핑되는지 확인하는 **정성적 검증(Qualitative Evaluation)**으로 전환하여 실효성을 입증했습니다.
